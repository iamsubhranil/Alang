...
static Keyword keywords[] = {{"Begin", 5, TOKEN_BEGIN},
                             {"End", 3, TOKEN_END},
...
static void fstack_free() {
	int i = 0;
	while(i < fsp) memfree(fileStack[i++]);
	memfree(fileStack);
}
...
static char *str_part(const char *source, int start) {
	...
	while(source[start + i] != '\n' && source[start + i] != '\r' &&
	      source[start + i] != '\0') {
		//        printf(debug("Current char : [%c] i : %d"), source[start+i],
		//        i);
...
static Token scanToken() {
	...
	switch(c) {
		...
		case '\n': scanner.line++; return makeToken(TOKEN_NEWLINE);
		...
		default:
			lnerr("Unexpected character %c!",
			      (Token){TOKEN_EOF, NULL, 0, scanner.line, scanner.fileName},
			      c);
			se++;
			return makeToken(TOKEN_EOF);
	}
...
static void insertList(TokenList **head, TokenList **prev,
                       TokenList *toInsert) {
	if(*head == NULL)
		(*head) = toInsert;
	else
