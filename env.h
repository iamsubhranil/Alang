...
// static inline void env_implicit_put(uint32_t key, Data value, Environment
// *env);
...
#define rec_new()                                                \
	freeRecordPointer == 0 ? (Record *)mallocate(sizeof(Record)) \
	                       : freeRecords[--freeRecordPointer]
#define rec_free(r)                                \
	{                                              \
		if(freeRecordPointer < MAX_FREE_RECORDS) { \
...
#define ienv_new()                                      \
	freeEnvironmentPointer == 0                         \
	    ? (Environment *)mallocate(sizeof(Environment)) \
	    : freeEnvironments[--freeEnvironmentPointer]
#define ienv_free(e)                                        \
	{                                                       \
		if(freeEnvironmentPointer < MAX_FREE_ENVIRONMENTS)  \
			freeEnvironments[freeEnvironmentPointer++] = e; \
		else                                                \
			memfree(e);                                     \
	}

#define env_new(parent) \
	(Environment) {     \
		NULL, parent    \
	}
...
static inline Record *env_match(uint32_t key, Environment *env) {
	...
	//    printf(debug("[Env:Match] Searching in %p[parent : %p] for [%s]"),
	//    env, env->parent, str_get(key));
	while(top != NULL) {
